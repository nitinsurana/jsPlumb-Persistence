...
//        connection1.getConnector().setPath(connections[i].path);
//        console.log(JSON.stringify(connections[i].overlays));
//        if (typeof connections[i].segment !== undefined) {
//            connection1.getConnector().setSegments(connections[i].segment);
//        }
        connections[i].overlays.forEach(function(overlay) {
            connection1.addOverlay([overlay.type, overlay]);
        });
//         connections[i].endpoint.forEach(function(overlay) {
//            connection1.addOverlay([overlay.type, overlay]);
//        });
...
//                    {url: endpoint.canvas.src},{endpoint.anchor.type}]);
